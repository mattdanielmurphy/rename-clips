...
const stemIdsToRenameDb = new JSONdb('stem-ids-to-fix.json')
const stemIdsToRename = Object.values(stemIdsToRenameDb.JSON())

function renameStemOrSample(
	filename: string,
	sampleOrStemName: string,
	correctedGhostNumber: string,
	wronglyCorrectedGhostNumber: string,
	pathToContainingDir: string,
) {
	let newFilename = ''
...
async function zipStems() {
	...
	for (const [i, sampleFilename] of Object.entries(samples)) {
		...
		const correctedGhostNumber = String(+uncorrectedGhostNumber + 300)
		const wronglyCorrectedGhostNumber = String(+uncorrectedGhostNumber - 700)
